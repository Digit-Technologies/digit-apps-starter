...
Prefer the React hooks from `@sutton/lib-frontend` — they wrap the harness client and
...
Types for `window.DigitHost` are exported from `@sutton/lib-frontend` (`DigitHost`,
...
- Pair with `@sutton/lib-backend` on the Worker (`createHandler`, `backendPath`, `ok` / `err`)

```js
import { AppErrorCode } from '@sutton/lib-common';
import { backendPath, createHandler, ok, err, requireEnv } from '@sutton/lib-backend';
...
Import codes / validation from `@sutton/lib-common`; Worker Response helpers from
`@sutton/lib-backend` (no re-exports between packages).
...
`data-theme` and `lang` are also set on `<html>`. Apps using `@sutton/lib-frontend` get
