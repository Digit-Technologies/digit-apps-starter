...
Call `getHostCapabilities` before writing the code — it returns the method names and their
params, so you never guess one. Then call `invoke` directly: do not guard it with a runtime
