...
Reference Sutton app that exercises the main platform surfaces in one place:
...
- `npm run pack` — `digit-app pack`: build + `app.zip` for Sutton upload

Local Sutton preview is not supported yet (no local Worker / env / D1 harness).

### What `pack` puts in `app.zip`

```
manifest.json             # Sutton publish config (zip root)
frontend/                 # Sutton deploy
backend/                  # Sutton deploy
