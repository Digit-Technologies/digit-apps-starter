...
Local `npm run dev` has no Sutton harness, Worker, or D1; tabs that call Sutton or the
...
```
Build a full-featured reference Sutton app we can use as the default example for the
starter repo. It should exercise theme/MUI via DigitThemeProvider, shared error handling
UI, Sutton GraphQL, a Worker calling a public API, secrets-backed third-party HTTP, D1
...
Make sure pack produces a zip we can publish and later rehydrate: Sutton deploy assets
