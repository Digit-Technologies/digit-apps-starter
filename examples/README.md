...
**Stack:** React + MUI + [`@heysutton/lib-frontend`](../packages/lib-frontend). Workers use
[`@heysutton/lib-backend`](../packages/lib-backend) + [`@heysutton/lib-common`](../packages/lib-common)
(codes / validation — depend directly; packages do not re-export each other). Tooling:
[`@heysutton/lib-build`](../packages/lib-build) (`digit-app pack`).
...
- Mount to `#root`
- Harness types come from `@heysutton/lib-frontend` — no local `digit.d.ts`
- Prefer `useDigitApiQuery` / `useBackendQuery` (and mutations) from `@heysutton/lib-frontend`
...
- `npm run pack` → `app.zip` with root `manifest.json` + `frontend/` (+ `backend/`) for
  ...
  `@heysutton/lib-*` incl. `lib-build` — not deployed)
- Do not copy Vite configs or pack scripts into each example — use `@heysutton/lib-build`
