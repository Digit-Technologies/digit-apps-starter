...
Depends on [`@sutton/lib-common`](../lib-common) internally. Apps should also depend on
`@sutton/lib-common` and import codes / validation from there — this package does **not**
re-export them.
...
import { backendPath, createHandler, requireEnv, ok, err } from '@sutton/lib-backend';
...
```js
import { AppErrorCode } from '@sutton/lib-common';
import { ok, err } from '@sutton/lib-backend';
...
Import parsers from `@sutton/lib-common`, then map failures with `err` from this package:

```js
import { parseJsonResponse, requiredString, optionalString } from '@sutton/lib-common';
import { err, ok } from '@sutton/lib-backend';
...
```js
import { createHandler, digitJobs, ok } from '@sutton/lib-backend';
...
```js
import { createHandler, requireEnv, verifyWebhookSignature } from '@sutton/lib-backend';
...
Use `@sutton/lib-build` (`digit-app pack`) — do not add a per-app Vite backend config.
...
{
  "dependencies": {
    "@sutton/lib-backend": "file:../../packages/lib-backend",
